...
int translate_data_to_points(const DataSource data_source, Vector2* points, const Bounds bounds) {
    const int start = find_first_visible(data_source.data, data_source.count, bounds.start_x);
    if (start == -1) return 0;
    ...
    for (int i = 0; i < visible_count; i++) {
        if (data_source.data[i + start].x > bounds.end_x) {
            return i;
        }
        points[i].x = (data_source.data[i + start].x - bounds.start_x) * x_factor + PLOT_OFFSET_X;
        points[i].y = (data_source.data[i + start].y + bounds.start_y) * y_factor + PLOT_OFFSET_Y;
    }
