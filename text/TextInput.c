...
text_change_t TextInput(const float x, const float y, char* buffer, int* len, const int max_len, const bool is_focused) {
    ...
    const float width = (FONT_SIZE * (max_len + 1) + 2 * PADDING) * .5f;
    DrawRectangle(x, y, width, PADDING + PADDING + FONT_SIZE, WHITE);
    DrawRectangleLines(x, y, width, PADDING + PADDING + FONT_SIZE, BLACK);
